...
export function SideNavBar() {
  ...
  return (
    <>
      <aside className="hidden md:flex fixed left-4 top-20 bottom-12 w-16 group hover:w-64 transition-all duration-300 z-40 bg-surface-container-high rounded-xl border border-outline-variant/20 shadow-xl flex-col overflow-hidden">
      ...
      <nav className="flex md:hidden fixed bottom-0 left-0 right-0 h-16 z-40 bg-surface-container-high border-t border-outline-variant/20 items-center justify-around px-2">
        {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
          const isActive = pathname === href;
          return (
            <Link
              key={href}
              href={href}
              className={`flex flex-col items-center justify-center gap-1 px-3 py-1.5 rounded-lg transition-all ${
